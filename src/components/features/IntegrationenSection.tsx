...
import { Cloud, Code, Package } from "lucide-react";

const IntegrationenSection = () => {
  const { t } = useTranslation();
  
    const features = [
        {
            icon: <Cloud className="h-8 w-8 text-apple" />,
            title: t('integration.api.title'),
            description: t('integration.api.description')
        },
        {
            icon: <Code className="h-8 w-8 text-apple" />,
            title: t('integration.developer.title'),
            description: t('integration.developer.description')
        },
        {
            icon: <Package className="h-8 w-8 text-apple" />,
            title: t('integration.marketplace.title'),
            description: t('integration.marketplace.description')
        }
    ];

  return (
    <section id={SECTION_IDS.integration} className="py-16 bg-white">
